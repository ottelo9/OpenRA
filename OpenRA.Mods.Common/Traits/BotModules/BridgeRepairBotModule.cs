...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class BridgeRepairBotModuleInfo : ConditionalTraitInfo
	{
		...
		public readonly HashSet<string> RepairActorTypes = new();
	...
	public class BridgeRepairBotModule : ConditionalTrait<BridgeRepairBotModuleInfo>, IBotTick
	{
		...
		readonly List<Actor> activeRepairers = new();
		...
		{
			if (Info.RepairActorTypes.Count == 0 || player.WinState != WinState.Undefined)
				return;
		...
		Target SafePath(Actor repairer, Actor target)
		{
			...
			var path = mobile.PathFinder.FindPathToTargetCell(
				repairer, new[] { repairer.Location }, target.Location, BlockedByActor.None,
